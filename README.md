...
5. #Настройка API Генерации Изображений
   ...
   Для этого выполните следующие шаги:
      ...
      *Добавьте ваш API ключ в этот файл следующим образом: ```bash API_KEY=your-api-key```

6. #Настрйока PORT:
   В файле .env напишите на каком порту запуститься ваше приложение ```bash PORT = 5000```
...
8. #Тестирование Конечных Точек:
   ...
   Пример с использованием в cmd с curl: ```bash curl -X POST http://localhost:5000/generate -H "Content-Type: application/json" -d "{\"prompt\": \"A beautiful sunset\", \"userId\": \"1234\"}"```
...
9.#Получение Изображений Пользователя 
  ...
  Пример с использованием в cmd с curl:```bash curl -X GET "http://localhost:5000/images?userId=1234"```
